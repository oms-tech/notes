...
$$
E[Y] = \frac{42}{24} \left(\frac{3 - 1 - (-3+1)}{9} \right) = \frac{42}{24} \times \frac{4}{9} = \frac{7}{9}
$$
...
$$
\begin{alignedat}{1}
\text{Cov}(X,X) & = E[X \times X] - E[X]E[X] \\[2ex]
& = E[X^2] - (E[X])^2 \\[2ex]
& = \text{Var}(X)
\end{alignedat}
$$
...
$$
E[XY] = (2 \times 40 \times 0.00) + (2 \times 50 \times 0.15) + ... + (4 \times 60 \times 0.1) = 140
$$
...
If $X_1$ and $X_2$ are *independent* with $X_i \sim \text{Nor}(\mu_i,\sigma_i^2), i = 1,2$, then $X_1 + X_2 \sim \text{Nor}(\mu_1 + \mu_2, \sigma_1^2 + \sigma_2^2)$.
...
$$
2X ~ \sim (\text{Nor}(2 \times 3, 2 \times 2 \times 4) \equiv \text{Nor}(6, 16)
$$

Now, let's find the distribution of $-3Y + 1$:

$$
-3Y + 1 \sim (\text{Nor}(-3 \times 4 + 1, -3 \times -3 \times 6) \equiv \text{Nor}(-11, 54)
$$
...
$$
\bar X_n \equiv \frac{1}{n}\sum_{i =1}\left[X_i \sim \text{Nor}(\mu, \sigma^2) \right] \sim \text{Nor}(\mu, \sigma^2 / n)
$$
...
Erlang random variables have cdf's, which we can use - directly or through software such as [Minitab](minitab.com/en-us/) - to obtain the *exact* value of the probability above:
...
& = \frac{\theta^2 \times n(n+2)}{3n\theta^2} \\[2ex]
& = \frac{n(n+2)}{3n} \\[2ex]
& = \frac{n+2}{3} \\[2ex]
\end{alignedat}
$$
...
$$
L(\lambda) =  (\lambda e^{-\lambda x_1}) \times (\lambda e^{-\lambda x_2}) \times \dots \times (\lambda e^{-\lambda x_n})
$$

We can pull out a $\lambda^n$ term:

$$
L(\lambda) = \lambda^n \times (e^{-\lambda x_1}) \times (e^{-\lambda x_2}) \times \dots \times (e^{-\lambda x_n})
$$
...
$$
\bar X_n - z_{\alpha/2}\sqrt{\frac{\sigma^2}{n}} \leq \mu \leq \bar X_n + z_{\alpha/2}\sqrt{\frac{\sigma^2}{n}}
$$
...
$$
\bar X_n - t_{\alpha/2,n-1}\sqrt{\frac{S^2}{n}} \leq \mu \leq \bar X_n + t_{\alpha/2,n-1}\sqrt{\frac{S^2}{n}}
$$
